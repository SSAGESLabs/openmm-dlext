...
template <typename T> constexpr DLDeviceType dlDeviceType();

template <> constexpr DLDeviceType dlDeviceType<ReferencePlatformData>() {
  return kDLCPU;
}

#ifdef OPENMM_BUILD_CUDA_LIB
template <> constexpr DLDeviceType dlDeviceType<CudaPlatformData>() {
  return kDLGPU;
}
#endif

template <typename T> T &ContextView::platformData() const {
