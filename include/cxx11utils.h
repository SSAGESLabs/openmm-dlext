...
template <typename T, typename Object> using Member = T Object::*;
...
template <typename T, typename... Args> UPtr<T> make_unique(Args &&...args) {
  return UPtr<T>(new T(std::forward<Args>(args)...));
}

template <typename T, typename Object, typename... Args>
T call_member(Object &object, MemberFunction<T, Object, Args...> member,
              Args &&...args) {
...
template <typename D, typename B> void assert_is_base_of() {
  static_assert(std::is_base_of<D, B>::value, "Invalid class passed");
}

// TODO: Check that T can be converted into S
template <typename S, typename T> bool contains(T &needle, Set<S> &haystack) {
